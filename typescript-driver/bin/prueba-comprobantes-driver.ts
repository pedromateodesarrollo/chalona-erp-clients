// CLI prueba: descarga driver TS activo, lo carga, corre batería de casos.
//
// Configura conexión vía env:
//   PG_HOST=localhost PG_PORT=5432 PG_DB=midb PG_USER=postgres PG_PASS=secret \
//   ENTORNO=test node dist/bin/prueba-comprobantes-driver.js
...
const entorno = (process.env.ENTORNO ?? 'test') as 'test' | 'produccion';

const casos: Array<{ nombre: string; comprobante: Record<string, unknown>; esperado_ok: boolean }> = [
  { nombre: 'tipo 31 OK',
...
async function main() {
  ...
  console.log(`== Lookup driver TS @ ${entorno} (${conn.host}:${conn.port}/${conn.database})`);
  const meta = await src.lookup();
  if (!meta) {
    console.error(`No hay driver TS activo en ${entorno}`);
    process.exit(1);
  }
